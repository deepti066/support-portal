        ...
        <div class="card-body">
            <form action="{{ route("admin.tickets.store") }}" method="POST" enctype="multipart/form-data">
                ...
                <div id="technical-person-group"
                     ...
                     style="display: none;">
                    <label for="technical_person">{{ trans('cruds.ticket.fields.technical_person') }}</label>
                    <select name="technical_person_id" id="technical_person" class="form-control select2">
                        @foreach($users as $user)
                            <option
                                value="{{ $user->id }}" {{ old('technical_person_id') == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                        @endforeach
                    </select>
