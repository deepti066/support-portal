        ...
        <div class="card-body">
            <form action="{{ route("admin.tickets.update", [$ticket->id]) }}" method="POST"
                ...
                <div>
                    ...
                    <select name="inventory_ids[]" id="inventory_id" class="form-control select2" multiple required>

                        @foreach ($inventories as $id => $product_name)
                            <option value="{{ $id }}"
                                {{ in_array($id, old('inventory_ids', $ticket->inventories->pluck('id')->toArray())) ? 'selected' : '' }}>
                                {{ $product_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group {{ $errors->has('priority_id') ? 'has-error' : '' }}">
                    ...
                    <select name="priority_id" id="priority" class="form-control select2" required>
                        @foreach($priorities as $id => $priority)
                            <option
                                value="{{ $id }}" {{ (isset($ticket) && $ticket->priority ? $ticket->priority->id : old('priority_id')) == $id ? 'selected' : '' }}>{{ $priority }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group {{ $errors->has('category_id') ? 'has-error' : '' }}">
                    ...
                    <select name="category_id" id="category" class="form-control select2" required>
                        @foreach($categories as $id => $category)
                            <option
                                value="{{ $id }}" {{ (isset($ticket) && $ticket->category ? $ticket->category->id : old('category_id')) == $id ? 'selected' : '' }}>{{ $category }}</option>
                        @endforeach
                    </select>
                ...
                <div id="technical-person-group"
                     class="form-group {{ $errors->has('technical_person_id') ? 'has-error' : '' }}"
                     style="display: none;">
                    <label
                        for="assign_to_technical_person_id">{{ trans('cruds.ticket.fields.technical_person') }}</label>
                    <select name="assign_to_technical_person_id" id="technical_person" class="form-control select2">
                        @foreach($users as $user)
                            <option
                                value="{{ $user->id }}" {{ (isset($ticket) && $ticket->technical_person_id ? $users->id : old('assign_to_technical_person_id')) == $id ? 'selected' : '' }}>{{ $user->name }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('technical_person_id'))
                        <em class="invalid-feedback">
                            {{ $errors->first('technical_person_id') }}
                        </em>
                    @endif
                </div>
                {{--    End        --}}
                    ...
                    <div class="form-group {{ $errors->has('assigned_to_user_id') ? 'has-error' : '' }}">
                        ...
                        <select name="assigned_to_user_id" id="assigned_to_user" class="form-control select2">
                            @foreach($assigned_to_users as $id => $assigned_to_user)
                                <option
                                    value="{{ $id }}" {{ (isset($ticket) && $ticket->assigned_to_user ? $ticket->assigned_to_user->id : old('assigned_to_user_id')) == $id ? 'selected' : '' }}>{{ $assigned_to_user }}</option>
                            @endforeach
                        </select>
